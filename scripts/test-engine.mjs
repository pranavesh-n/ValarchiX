import { calculateGoalX } from "../src/lib/engine/goalx.js";
import { calculateFinancialDna } from "../src/lib/engine/dna.js";
import { calculateTimeMachineScenario } from "../src/lib/engine/timemachine.js";
import { calculateXirr } from "../src/lib/engine/xirr.js";
import { formatINR, formatINRWords } from "../src/lib/engine/numeric.js";
...
// Future cost: 20L * (1.10)^10 = 51.87L
...
assert(goalResult.recommendedPaths.length >= 3, "GoalX Reverse Mode generates 3+ actionable paths (SIP Increase, Lump Sum, Tenure Extension)");
...
const extremeTwin = {
  ...dummyTwin,
  income: { monthlySalary: 1, secondaryMonthlyIncome: 0, expectedAnnualGrowthPct: 0, stabilityRating: "low" },
...
// 3. XIRR Newton-Raphson Engine Tests
console.log("\n📈 Testing XIRR Newton-Raphson Cashflow Engine...");
const cashflows = [
  { amount: -100000, date: new Date("2023-01-01") },
  { amount: -50000, date: new Date("2023-07-01") },
  { amount: 180000, date: new Date("2024-01-01") }
];
const xirrVal = calculateXirr(cashflows);
assert(!isNaN(xirrVal) && xirrVal > 0.10 && xirrVal < 0.40, "XIRR calculates realistic annualized return (~25-30%)", `Got: ${xirrVal}`);

// 4. Numeric Formatting Edge Cases (Adaptive Logarithmic Range)
console.log("\n🔢 Testing Numeric Formatting (Adaptive Scale: ₹1 to ₹1,000Cr+)...");
assert(formatINR(1) === "₹1", "Formats ₹1 correctly");
assert(formatINR(100000) === "₹1,00000" || formatINR(100000).includes("1,00,000"), "Formats ₹1L correctly");
